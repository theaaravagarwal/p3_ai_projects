...
DEFAULT_BULK_ROOT = os.path.join(PROJECT_ROOT, "secretdata")
...
class DatasetEvalError(RuntimeError):
    pass
...
def _require_sklearn_metrics() -> None:
    if any(
        ...
        raise DatasetEvalError(
            "scikit-learn is required for dataset metrics. Install `scikit-learn` and retry."
        )
...
def _label_from_path(root: Path, image_path: Path) -> Optional[str]:
    try:
    ...
    if not rel_parts:
        return None
    candidate = _normalize_label(rel_parts[0])
    return candidate if candidate in CLASS_NAMES else None


def _format_probabilities(probabilities: np.ndarray) -> str:
    pieces = []
    for class_name, score in zip(CLASS_NAMES, probabilities):
        pieces.append(f"{class_name}:{float(score) * 100.0:.1f}%")
    return " ".join(pieces)


def _print_prediction_tree(records: list[ImagePrediction], root: Path) -> None:
    print()
    print("Prediction tree")
    print(root.name + "/")
    grouped: dict[str, list[ImagePrediction]] = {}
    for record in records:
        grouped.setdefault(record.actual_label, []).append(record)
    ...
    for class_name in CLASS_NAMES:
        ...
        wrong_records = [
            ...
            if (not record.detected) or record.predicted_label != record.actual_label
        ]
        ...
        for record in wrong_records:
            rel_display = Path(record.rel_path).as_posix()
            if not record.detected:
                print(f"    +-- {rel_display} -> no hand detected")
                continue
            print(
                f"    +-- {rel_display} -> actual={record.actual_label}, "
                f"predicted={record.predicted_label}, confidence={record.confidence * 100.0:.1f}%"
            )
            print(f"        {_format_probabilities(record.probabilities)}")


def _print_metrics(records: list[ImagePrediction]) -> None:
    _require_sklearn_metrics()
    if not records:
        raise DatasetEvalError("No labeled images were processed successfully.")

    detected_records = [record for record in records if record.detected]
    y_true = [record.actual_label for record in records]
    y_pred = [record.predicted_label for record in records]
    ...
    print(f"  f1_weighted: {f1_score(y_true, y_pred, average='weighted', zero_division=0):.4f}")
    print(f"  detection_rate: {len(detected_records) / len(records):.4f}")
    print(f"  no_detection_count: {len(records) - len(detected_records)}")

    cm_labels = [*CLASS_NAMES, NO_DETECTION_LABEL]
    cm = confusion_matrix(y_true, y_pred, labels=cm_labels)
    per_class_acc = []
    for idx, class_name in enumerate(CLASS_NAMES):
        support = int(cm[idx].sum())
        class_acc = float(cm[idx, idx] / support) if support else 0.0
        per_class_acc.append((class_name, class_acc, support))

    print()
    print("Per-class summary")
    print("  class  precision  recall  f1-score  support  class-acc")
    ...
    for idx, class_name in enumerate(CLASS_NAMES):
        class_acc = per_class_acc[idx][1]
        print(
            f"  {class_name:>5}  {precision[idx]:>9.4f}  {recall[idx]:>6.4f}"
            f"  {f1[idx]:>8.4f}  {int(support[idx]):>7d}  {class_acc:>9.4f}"
        )
    ...
    header = "      " + " ".join(f"{name:>6}" for name in [*CLASS_NAMES, "MISS"])
    print(header)
    for idx, class_name in enumerate(CLASS_NAMES):
        row = " ".join(f"{int(value):>6d}" for value in cm[idx])
        print(f"  {class_name:>2}  {row}")


def _collect_dataset_images(root: str) -> list[tuple[Path, str]]:
    root_path = Path(os.path.expanduser(root)).resolve()
    if not root_path.is_dir():
        raise DatasetEvalError(f"Dataset directory does not exist: {root}")

    files = [path for path in sorted(root_path.rglob("*")) if is_image_file(path)]
    if not files:
        raise DatasetEvalError(f"No supported image files found under: {root}")

    labeled_files: list[tuple[Path, str]] = []
    for path in files:
        label = _label_from_path(root_path, path.resolve())
        if label is None:
            continue
        labeled_files.append((path.resolve(), label))

    if not labeled_files:
        raise DatasetEvalError(
            f"No labeled images were found under {root}. Expected class folders {CLASS_NAMES}."
        )
    return labeled_files


def evaluate_secretdata(
    model: nn.Module,
    root: str,
    device: torch.device,
    cropper: Optional[MediaPipeHandCropper],
) -> list[ImagePrediction]:
    image_entries = _collect_dataset_images(root)
    root_path = Path(os.path.expanduser(root)).resolve()
    records: list[ImagePrediction] = []

    print(f"Found {len(image_entries)} labeled image(s) under {root_path}")
    print("Processing recursively...")

    with torch.inference_mode():
        for index, (image_path, actual_label) in enumerate(image_entries, start=1):
            image = cv2.imread(str(image_path))
            if image is None:
                records.append(
                ImagePrediction(
                    image_path=str(image_path),
                    rel_path=os.path.relpath(str(image_path), str(root_path)),
                    actual_label=actual_label,
                    predicted_label=NO_DETECTION_LABEL,
                    confidence=0.0,
                    probabilities=np.zeros(len(CLASS_NAMES), dtype=np.float32),
                    detected=False,
                        roi_source="Unreadable",
                    )
                )
                print(f"[{index}/{len(image_entries)}] {image_path.name} -> unreadable")
                continue
            ...
            if crop.size == 0:
                ...
                print(f"[{index}/{len(image_entries)}] {image_path.name} -> no hand detected")
                continue
            ...
            predicted_label, confidence = decode_prediction(probs)
            records.append(
                ImagePrediction(
                    image_path=str(image_path),
                    rel_path=os.path.relpath(str(image_path), str(root_path)),
            ...
            status = "OK" if predicted_label == actual_label else "WRONG"
            print(
                f"[{index}/{len(image_entries)}] {image_path.name} -> "
                f"actual={actual_label}, predicted={predicted_label}, "
                f"confidence={confidence * 100.0:.1f}% [{source}] {status}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Root directory used for the default bulk dataset. "
    ...
    parser.add_argument(
        "--tree",
        action="store_true",
        help="Print a tree-style report of misclassified or undetected images with confidences.",
    )
    parser.add_argument(
        "--disable-mediapipe",
        action="store_true",
...
def find_labeled_bulk_image_files(directory: str) -> list[tuple[str, str]]:
    root = Path(os.path.expanduser(directory))
    if not root.is_dir():
        raise RuntimeError(f"Bulk directory does not exist or is not a directory: {directory}")
    files = [path for path in sorted(root.rglob("*")) if is_image_file(path)]
    ...
    for path in files:
        label = _label_from_path(root.resolve(), path.resolve())
        if label is not None:
            labeled_files.append((str(path), label))
    if not labeled_files:
        raise RuntimeError(f"No labeled image files found in: {directory}")
    return labeled_files
...
def run_bulk_mode(
    ...
    labeled_image_files = find_labeled_bulk_image_files(directory)
    image_files = [path for path, _ in labeled_image_files]
    labels_by_path = dict(labeled_image_files)
    selected_to_show = parse_bulk_show_selection(bulk_show_selection, image_files)
    root_path = Path(os.path.expanduser(directory)).resolve()
    records: list[ImagePrediction] = []

    print(f"Found {len(image_files)} labeled image(s) in {root_path}")
    ...
    with torch.inference_mode():
        for index, (image_path, image) in enumerate(iter_loaded_images(image_files), start=1):
            actual_label = labels_by_path[image_path]
            if image is None:
            ...
            print(
                ...
                f"actual={actual_label}, predicted={label}, "
                f"confidence={confidence * 100.0:.1f}% [{source}]"
            )

            if tree or not selected_to_show:
                continue

            abs_path = os.path.abspath(image_path)
            if abs_path not in selected_to_show:
                continue
    ...
    if tree:
        _print_prediction_tree(records, root_path)
