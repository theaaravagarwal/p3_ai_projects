...
class LegacyResidualASLBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1):
        super().__init__()
        self.conv1 = nn.Conv2d(
            in_channels,
...
def build_legacy_model(num_classes: int) -> nn.Module:
    return nn.Sequential(
        ...
        nn.BatchNorm2d(32),
        nn.GELU(),
        LegacyResidualASLBlock(32, 64, stride=2),
        LegacyResidualASLBlock(64, 64),
        LegacyResidualASLBlock(64, 64),
        nn.Dropout2d(0.08),
        LegacyResidualASLBlock(64, 128, stride=2),
        LegacyResidualASLBlock(128, 128),
        LegacyResidualASLBlock(128, 128),
        nn.Dropout2d(0.12),
        LegacyResidualASLBlock(128, 192, stride=2),
        LegacyResidualASLBlock(192, 192),
        LegacyResidualASLBlock(192, 192),
        nn.Dropout2d(0.16),
        LegacyResidualASLBlock(192, 256, stride=2),
        LegacyResidualASLBlock(256, 256),
        LegacyResidualASLBlock(256, 256),
...
def build_legacy_cnn_model(num_classes: int) -> nn.Module:
    return nn.Sequential(
        nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False),
        nn.BatchNorm2d(32),
        nn.ReLU(inplace=True),
...
def load_checkpoint_state_dict(weights_path: str, device: torch.device):
    ...
    raise ValueError(f"{weights_path} does not look like a supported PyTorch checkpoint.")


def detect_checkpoint_variant(state_dict) -> str:
    if any(".spatial.conv.weight" in key for key in state_dict):
        return "legacy_cbam"
    if any(key.startswith("3.weight") for key in state_dict):
        return "legacy_cnn"
    return "current"
...
def infer_num_classes(state_dict) -> int:
    classifier_weights = [
        value for key, value in state_dict.items() if key.endswith(".weight") and value.ndim == 2
    ]
    if not classifier_weights:
        raise ValueError("Could not infer classifier size from checkpoint.")
    return classifier_weights[-1].shape[0]


def resolve_class_names(num_classes: int) -> List[str]:
...
def load_model(weights_path: str, device: torch.device):
    ...
    variant = detect_checkpoint_variant(state_dict)
    num_classes = infer_num_classes(state_dict)

    if variant == "legacy_cbam":
        model = build_legacy_model(num_classes).to(device)
    elif variant == "legacy_cnn":
        model = build_legacy_cnn_model(num_classes).to(device)
    else:
        model = build_model(num_classes).to(device)

    model.load_state_dict(state_dict)
    model.eval()
    return model, resolve_class_names(num_classes), variant
...
def predict_topk(
    model: nn.Module, input_tensor: torch.Tensor, class_names: List[str], k: int = 3
...
def crop_padded_square(frame_bgr, bbox, pad_ratio: float = 0.2):
    x, y, w, h = bbox
    frame_h, frame_w = frame_bgr.shape[:2]
    side = max(w, h)
    pad = int(side * pad_ratio)
    half_side = (side + 2 * pad) / 2.0

    cx = x + (w / 2.0)
    cy = y + (h / 2.0)

    x1 = max(0, int(round(cx - half_side)))
    y1 = max(0, int(round(cy - half_side)))
    x2 = min(frame_w, int(round(cx + half_side)))
    y2 = min(frame_h, int(round(cy + half_side)))
...
def main():
    ...
    model, class_names, model_variant = load_model(args.model_path, device)
    ...
    try:
        ...
        while True:
            ...
            if bbox is not None:
                x, y, w, h = bbox
                roi = crop_padded_square(frame, bbox)
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 220, 255), 2)
            else:
                roi = frame

            input_tensor = preprocess_frame(roi, device)
            top3 = predict_topk(model, input_tensor, class_names, k=3)
